...
let handleLayout = (event, self) => {
  ...
  SetOrigin({x: layout.x +. 0.5 *. layout.width, y: layout.y +. 0.5 *. layout.height})
...
let make = (~ticks: Touch.ticks('a), ~onRelease: option(Touch.releaseHandler('a))=?, children) => {
  ...
  initialState: () => {
    ...
    {
      panResponder: Touch.panResponderAboutOrigin(~origin={x: 0., y: 0.}, ~angle, ~ticks, ()),
      angle
    }
  },
  reducer: (action, state) =>
    ...
    | SetOrigin(origin) =>
      ReasonReact.Update({
        ...state,
        panResponder:
          Touch.panResponderAboutOrigin(~origin, ~angle=state.angle, ~ticks, ~onRelease?, ())
      })
    },
  render: (self) =>
    <View
      ...
      <Animated.View style=(rotatingViewStyle(self.state.angle))> ...children </Animated.View>
    </View>
};
